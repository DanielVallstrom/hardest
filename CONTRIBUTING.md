...
After you find a bound, you can milk the solution: Use the milk mode, -1. You'll want to base your milking on the solution, but e.g. up the number of sub-searches that you take the most promising search from, i.e. upping -B settings. For example, if -b 3 was used to find the bound, try combinations around that setting, for a few levels, e.g. -1 -b 3 -i 3 -B 0:4 -B 1:4 -B 2:4 -B 3:4 -2 b-2 Again, see [`best_known_bounds.csv`](best_known_bounds.csv).
